...
- `ApplicationInstaller::install()` only auto-seeded the "Calendario" system entity — "Documenti" and "E-mail" (both `is_system => true`, same category, added later on 2026-07-31 and 2026-08-09 respectively) were never added to the install wizard's seed list, so every fresh install was missing two of its three built-in system entities and their menu entries. Demo/business entities (Clienti, Fatture, Preventivi, Ticket, ...) are unaffected — those stay opt-in by design, called from a host's own `DatabaseSeeder`.
