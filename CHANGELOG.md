...
- Restored the Tabler-themed override of `fazzinipierluigi/laraccoon-layouts`' dropdown view (`resources/views/vendor/raccoon-layouts/dropdown.blade.php`, `form-select` + real Bootstrap `dropdown-menu`, Tom-Select-aware option rebuilding) as a package-published file — it existed before (added 2026-07-12) but lived under the old standalone app's `resources/views/vendor/`, dropped in `0.2.0`'s app-shell removal like `public/logo.svg`. Every host was silently falling back to the vendor package's own unstyled, BEM-only markup (which it ships on purpose — see its README — expecting a host to override it). Published via `crm-assets`, since Laravel's view finder checks `resource_path('views/vendor/{namespace}/...')` before a package's own registered path for any namespace, not just this package's own.
