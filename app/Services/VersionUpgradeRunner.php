...
/**
 * Resolves and runs the App\Services\Upgrades\UpgradeStep classes
 * registered in config('upgrades.steps') that fall between two app
 * versions, in the direction the update wizard needs.
 */
class VersionUpgradeRunner
{
    ...
    {
        $steps = array_map(fn (string $class) => app($class), config('upgrades.steps', []));
