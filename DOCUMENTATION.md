...
- Business rules: `UserController::destroy()` refuses to let a user delete their own account (compares `$user->id` to `$request->user()->id`). `RoleController`: system roles (`is_system=true`, e.g. `admin`) can't be deleted, and their `slug` is silently ignored on update (name still editable).
