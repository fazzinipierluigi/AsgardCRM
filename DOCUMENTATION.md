...
- Admin CRUD: `app/Http/Controllers/Admin/TranslationController.php`, routes `admin/translations/*` (same shape as Users/Roles — `data()` before `Route::resource(...)`), menu entry in `layouts/admin.blade.php`. `language` select is restricted to `array_keys(config('preferences.language.options'))` — the same language list already used for the user preference, so there's one canonical list of "supported languages" for the whole app.
- No is_admin-style special-casing here — any translation row can be freely created/edited/deleted.
...
- **`database/seeders/TranslationSeeder.php`** bulk-imports the `it`/`en` value for every string currently passed to `t()` across the views (~66 keys × 2 languages), called from `DatabaseSeeder::run()` so a plain `php artisan db:seed` (already the documented install step) populates it — "installation mass-imports the translations" per the standing project rule. Uses `Translation::upsert($rows, ['key','language'], ['value','updated_at'])` for one bulk query instead of 130+ individual `create()` calls; safe to re-run (updates existing rows, no duplicates). **When you add a new `t('some string')` call to a view, add its `it`/`en` entry to this seeder's `$strings` array** — otherwise `t()` just echoes the raw key back for that string, which is the "missing translation" fallback but does mean nobody actually translated it yet.
  - **Gotcha (hit once, fixed):** `Translation::upsert()` is a query-builder bulk operation — it does **not** fire Eloquent's `saved`/`deleted` model events, so `Translation::forgetCache()` (which those events normally trigger) never runs automatically. The seeder calls `Translation::forgetCache()` itself right after the `upsert()` call to compensate. Any other bulk/raw write path (`DB::table('translations')->...`, another `upsert()`, mass `update()` via the query builder) needs the same explicit `forgetCache()` call — only single-model `create()`/`update()`/`delete()` (as used by `TranslationController`, the admin CRUD) invalidate the cache on their own.
  - **Test-suite gotcha (hit once, fixed):** `Translation::$cache` is a `private static array`, so it survives across **all** test cases run in the same PHPUnit/Pest process (Feature and Browser tests each run in one process). A translation written directly via the query builder or via `upsert()` in one test could leave a stale cached value that a *later, unrelated* test then reads. Fixed by resetting it globally in `tests/Pest.php`: `->afterEach(fn () => Translation::forgetCache())` chained onto both the `Feature` and `Browser` `pest()->extend(...)` calls. This isn't a production concern under normal PHP-per-request execution (each request is a fresh process, so the static array starts empty) — it only bites long-lived processes (Octane, the Pest test runner) or, as here, any code path that bypasses Eloquent events.
...
## admin CRUD (Users / Roles / Translations)
...
- **Role slugs are auto-generated from the name on create**, never typed by hand: `RoleController::store()` computes `Str::slug($name)` and appends `-2`, `-3`, ... until unique (`uniqueSlug()` private method) — `StoreRoleRequest` has no `slug` rule at all. The create form (`resources/views/admin/roles/create.blade.php` → `_form.blade.php`) has no slug field; `_form.blade.php`'s slug block is now guarded by `@if ($role)` so it only appears on the edit form (where the slug is still editable for non-system, non-admin roles, per the existing `is_system` lock).
