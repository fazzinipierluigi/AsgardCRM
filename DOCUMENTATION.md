...
- No is_admin-style special-casing here — any translation row can be freely created/edited/deleted, including ones for keys used by the app's own chrome (nav labels etc, if that convention is adopted later — nothing currently wires static UI strings through `t()`, `__()` in Blade views is still the framework's own file-based translator, untouched by this feature).
