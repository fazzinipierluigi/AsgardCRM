...
<div class="tab-pane repeatable-row {{ $active ? 'show active' : '' }}" id="tab-pane-{{ $tabToken }}" role="tabpanel" data-tab-token="{{ $tabToken }}">
    <div class="cards-container row row-cards">
        @foreach ($cards as $card)
            @include('crm::admin.entities._card', ['tabToken' => $tabToken, 'cardToken' => $card->id, 'card' => $card, 'mandatory' => $mandatory && $loop->first, 'relationTargets' => $relationTargets, 'fieldTypes' => $fieldTypes, 'entity' => $entity])
        @endforeach
    </div>
