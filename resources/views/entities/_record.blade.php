            ...
            <div class="tab-pane show active" id="entity-record-tab-data" role="tabpanel">
                ...
                @if ($mode === 'edit')
                    <form action="{{ route('entities.update', [$entity, $record]) }}" method="POST" id="entity-record-form">
                        @csrf
                        @method('PUT')
                        @include('entities._form', ['entity' => $entity, 'record' => $record, 'relationOptions' => $relationOptions, 'productsBlockOptions' => $productsBlockOptions ?? []])
                    </form>
                @else
                    @include('entities._form', ['entity' => $entity, 'record' => $record, 'relationOptions' => $relationOptions, 'productsBlockOptions' => $productsBlockOptions ?? [], 'readonly' => true])
                @endif
            </div>
