        ...
        <div class="card-body">
            <form action="{{ route('admin.document-storage.update') }}" method="POST" id="document-storage-form">
                ...
                <fieldset data-storage-config="ftp" class="mb-3">
                    ...
                    @include('admin.document-storage._ftp_sftp_fields', ['value' => $value, 'isEdit' => $setting->exists, 'showSsl' => true, 'defaultPort' => 21, 'prefix' => 'ftp_'])
                </fieldset>

                <fieldset data-storage-config="sftp" class="mb-3">
                    <legend class="fs-4">{{ t('Server SFTP') }}</legend>
                    @include('admin.document-storage._ftp_sftp_fields', ['value' => $value, 'isEdit' => $setting->exists, 'showSsl' => false, 'defaultPort' => 22, 'prefix' => 'sftp_'])
                </fieldset>
            </form>
        </div>
    </div>
