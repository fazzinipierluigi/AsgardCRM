...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        ...
        >
            @foreach (\Fazzinipierluigi\AsgardCRM\Enums\ConnectorType::options() as $value2 => $label)
...
<div class="row">
    <div class="col-md-6 mb-3">
        ...
        <select id="sync_direction" name="sync_direction" class="form-select @error('sync_direction') is-invalid @enderror">
            @foreach (\Fazzinipierluigi\AsgardCRM\Enums\ConnectorSyncDirection::options() as $value2 => $label)
    ...
    <div class="col-md-6 mb-3">
        <label for="sync_interval_minutes" class="form-label">{{ t('Intervallo sincronizzazione (minuti)') }}</label>
        <input
            type="number"
            ...
            min="1"
            max="1440"
            value="{{ old('sync_interval_minutes', $connector?->sync_interval_minutes ?? 15) }}"
...
<fieldset data-connector-config="exchange_graph" class="mb-3">
    ...
    <div class="mb-3">
        ...
        <small class="form-hint">{{ t('Richiede un app registration Microsoft Entra con permesso applicativo Calendars.ReadWrite e consenso admin.') }}</small>
    </div>
</fieldset>

<fieldset data-connector-config="exchange_ews" class="mb-3">
    ...
    <div class="mb-3">
        ...
        <small class="form-hint">{{ t('Serve un account di servizio con diritto EWS Impersonation per sincronizzare più mailbox.') }}</small>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var fieldsets = document.querySelectorAll('[data-connector-config]');

        function syncVisibility() {
            var type = typeSelect.value;
            fieldsets.forEach(function (fieldset) {
                fieldset.style.display = fieldset.getAttribute('data-connector-config') === type ? '' : 'none';
            });
        }
