                ...
                <form action="{{ route('admin.entities.visibility.update', $entity) }}" method="POST" id="entity-visibility-form">
                    ...
                    <table class="table" data-testid="entity-visibility-table">
                        <thead>
                            <tr>
                                <th>{{ t('Ruolo') }}</th>
                                @foreach (\Fazzinipierluigi\AsgardCRM\Enums\EntityVisibilityLevel::options() as $value => $label)
                                    <th>{{ $label }}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($roles as $role)
                                @php
                                    $currentLevel = old("levels.{$role->id}", $currentLevels[$role->id] ?? \Fazzinipierluigi\AsgardCRM\Enums\EntityVisibilityLevel::OwnOnly->value);
                                @endphp
