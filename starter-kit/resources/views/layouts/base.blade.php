...
@section('menu')
    ...
    <div class="mt-auto">
        ...
        {{-- Punta alla gestione entita' (sempre presente, e' il
             package crm-core) invece che a admin.users.index: la
             gestione utenti/ruoli e' CRUD host-owned non incluso nello
             starter kit minimo (vedi layouts/admin.blade.php). --}}
        @can('admin.access')
            <ul class="navbar-nav pb-lg-3">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('admin.entities.index') }}" data-testid="menu-admin">
                        <span class="nav-link-icon">
                            {!! icon('settings-cog') !!}
                        </span>
