...
@section('menu')
    <ul class="navbar-nav">
        {{-- Utenti/Ruoli/Login provider sono CRUD amministrativi
             host-owned (mai spostati nel package crm-core, come
             l'intera Fase 3 ha stabilito) — lo starter kit minimo non
             li porta (stessa scelta di scope di Social/SAML/LDAP nel
             login), quindi ogni voce e' condizionale: appare da sola
             se il consumer aggiunge quei controller/rotte in futuro
             (vedi l'app AsgardCRM originale come riferimento). --}}
        @if (Route::has('admin.users.index') || Route::has('admin.roles.index') || Route::has('admin.login-providers.index'))
            ...
            @endif
            @if (Route::has('admin.login-providers.index'))
                <li class="nav-item">
